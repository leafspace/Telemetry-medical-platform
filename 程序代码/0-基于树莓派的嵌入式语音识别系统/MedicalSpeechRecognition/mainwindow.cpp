...
void MainWindow::ClickButton_Reco()
{
    ...
    if ((reco_f = recognitiononlineThread.uploadData("prammar.abnf", "dtt = abnf, sub = asr")) == false) {
        ...
        ui->textEdit_gram->clear();
        ui->textEdit_gram->append(recognitiononlineThread.getGrammarId());
    }


    result = recognitiononlineThread.recognition("wav/upload.wav", "sub = asr, result_type = plain, result_encoding = gbk");
    if (result == NULL) {
		while (!recognitiononlineThread.errorInfo.empty()) {
			QMessageBox::critical(this, tr("Error"), recognitiononlineThread.errorInfo.pop(), QMessageBox::Yes);
		}
    ...
    if(strlen(result) == 0){
        ui->textEdit_resu->clear();
        ui->textEdit_resu->append("No Resault!");
    }else{
        ui->textEdit_resu->clear();
        //ui->textEdit_resu->append(recognitiononlineThread.getTopResult());
        ui->textEdit_resu->append(result);
    }
}
